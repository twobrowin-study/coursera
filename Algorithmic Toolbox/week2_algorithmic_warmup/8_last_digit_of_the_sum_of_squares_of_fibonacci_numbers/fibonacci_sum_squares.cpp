...
uint64_t fibonacci_sum_squares_naive(uint64_t n) {
    if (n <= 1)
        return n;
...
uint64_t fibonacci_sum_squares_fast(uint64_t n) {
    if (n <= 1) {
        return n;
    }
    ...
    for (uint64_t i = 0; i < n % d + 1; i++) {
        inner_sum += pisano[i] * pisano[i];
    }

    uint64_t outer_sum = 0;
    if ( n / d > 0) {
        for (uint64_t i = 0; i < d; i++) {
            outer_sum += pisano[i] * pisano[i];
        }
        outer_sum *= n / d;
    }
...
void test_solution() {
    ...
    for (uint64_t n = 0; n < 30; ++n) {
        ...
        uint64_t fast = fibonacci_sum_squares_fast(n);
        uint64_t naive = fibonacci_sum_squares_naive(n);
